...
from format.twenty.TwentyPack import pac_boxes_twenty
    ...
    try:
        ...
        for i in range(box_num):
            ...
            for j in range(2):
                ...
                if (box_num - 1) == i:
                    # 返回下一个接受数量的开始
                    accept_index = pac_boxes_twenty(j, accept_index, total_index, sheet_read, box_sheet, num + 4, True, int(total_content), content_style)
                else:
                    accept_index = pac_boxes_twenty(j, accept_index, total_index, sheet_read, box_sheet, num + 4, False, int(total_content), content_style)
                    # =======================================================
                box_sheet.write_merge(num + 9, num + 9, 0, 6, '本箱内合计数量:' + str(shoes_num[i]) + '双', content_style)
